...
export default function RegisterPage() {
  ...
  const handleSelectOrg = (org: RnaAssociation) => {
    setSelectedOrg(org);
    setOrgSearch('');
    setOrgResults([]);
    setManualMode(false);
  ...
  const canProceedStep2 = role === 'PERSONAL' || selectedOrg || (manualMode && manualOrgName.length >= 2);
  const totalSteps = role === 'ORG_ADMIN' ? 3 : 2;

  const goNext = () => {
    if (step === 1 && role === 'PERSONAL') {
      ...
      <Card padding="lg" className="shadow-xl border-0">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-500 text-sm mt-2">
            ...
            {step === 3 && 'Vos informations personnelles'}
          </p>
        </div>

        {/* Progress dots */}
        <div className="flex items-center justify-center gap-2 mb-6">
          {[1, 2, 3].filter((s) => role === 'ORG_ADMIN' ? true : s !== 2).map((s) => (
            <div
              key={s}
              className={cn(
                'w-2 h-2 rounded-full transition-all duration-300',
                step === s ? 'w-6 bg-kooki-500' : step > s || (step === 3 && s === 1) ? 'bg-kooki-300' : 'bg-gray-200'
              )}
            />
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {/* STEP 2: Search organization (ORG_ADMIN only) */}
          {step === 2 && (
            <motion.div
              key="step2"
              initial={{ opacity: 0, x: -20 }}
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex gap-3">
                  <Button variant="ghost" size="lg" type="button" onClick={goBack} className="flex-1">
                    <ArrowLeft size={18} />
                    Retour
                  </Button>
                  <Button
